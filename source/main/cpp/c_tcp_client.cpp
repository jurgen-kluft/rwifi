...
namespace ncore
{
    namespace ntcp
    {
        ...
        static void s_poll_connected(tcp_client_t& c)
        {
            ...
            while (c.m_config->m_sock_ops.m_available(c.m_socket) > 0)
            {
                if (c.m_tcp_recv_expected == 0)
                {
                    ...
                    c.m_tcp_recv_expected = c.m_config->m_fmp_ops.m_process_hdr(c.m_tcp_recv_header, c.m_tcp_recv_header_size);

                    c.m_tcp_recv_offset = 0;
                    c.m_tcp_recv_buf    = c.m_config->m_recv_ops.m_acquire(c.m_tcp_recv_ctx, c.m_tcp_recv_expected);
                ...
                c.m_config->m_sock_ops.m_read(c.m_socket, c.m_tcp_recv_buf + c.m_tcp_recv_offset, chunk);
                c.m_tcp_recv_offset += chunk;

                if (c.m_tcp_recv_offset == c.m_tcp_recv_expected)
                {
                    c.m_config->m_recv_ops.m_commit(c.m_tcp_recv_ctx, c.m_tcp_recv_buf, c.m_tcp_recv_expected);
        ...
        {
            if (c.m_state != TCP_STATE_INACTIVE)
                return;
            c.m_state = TCP_STATE_DISCONNECTED;
        }
